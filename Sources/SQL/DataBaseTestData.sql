...
WHILE @@FETCH_STATUS = 0 BEGIN
  ...
  SET @i = 0;
  WHILE @i < @RandInt BEGIN
    INSERT INTO BACKOFFICE._FEEL_CLI_CLI (FCC_CLI_FROM_ID, FCC_CLI_TO_ID,FCC_FTY_ID)
    VALUES (@Id,
            (SELECT TOP 1 CLI_ID FROM BACKOFFICE._CLIENT WHERE CLI_ID <> @Id AND CLI_ID NOT IN (SELECT FCC_CLI_TO_ID FROM BACKOFFICE._FEEL_CLI_CLI WHERE FCC_CLI_FROM_ID = @Id) ORDER BY NEWID()),
        ...
        IF ROUND(RAND()*3,0) <> 0 BEGIN
          ...
          VALUES (@Id,
                  (SELECT TOP 1 DIS_ID
                   ...
                   WHERE DIS_ID IN (SELECT OFF_DIS_ID FROM BACKOFFICE._OFFER WHERE OFF_REC_ID = @Id2)
                     AND DIS_TYPE = 1
                   ORDER BY NEWID()),
                  @Id2);
        END
...
SELECT * FROM MANAGERAREA.DishWish;
SELECT * FROM MANAGERAREA.Feeling;
SELECT * FROM MANAGERAREA.Menu;
SELECT * FROM MANAGERAREA.Reception;
SELECT * FROM MANAGERAREA.Reservation;
SELECT * FROM MANAGERAREA.ReservedDish;
